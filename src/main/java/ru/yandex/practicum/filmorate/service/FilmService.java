...
public class FilmService {
     ...
     * Предварительно проверяет существование пользователей и наличие дружбы между ними.
    ...
    /**
     * Проверяет существование пользователей и наличие дружбы между ними.
     * <p>
     * Проверяет, существуют ли пользователи с идентификаторами {@code userId} и {@code friendId},
     * а также являются ли они друзьями.
     * <p>
     * Если какой-либо пользователь не найден, выбрасывается {@link UserNotFoundException}.
     * Если пользователи не являются друзьями, выбрасывается {@link ValidationException}.
     *
     * @param userId   идентификатор первого пользователя.
     * @param friendId идентификатор второго пользователя (друга).
     * @throws UserNotFoundException если один из пользователей не найден.
     * @throws ValidationException   если пользователи не являются друзьями.
     */
    private void validateFriendship(Integer userId, Integer friendId) {
        getExistingUser(userId);
        getExistingUser(friendId);
        if (!userStorage.areFriends(userId, friendId)) {
            String message = "Пользователи с id=" + userId + " и id=" + friendId + " не являются друзьями";
            log.warn(message);
